...
def a_star(start, target, map: Map):
    ...
    f = np.full(map.width * map.height, float('Inf'))
    f[map.toIndex(start)] = map.distance(start, target)
        ...
        for point, cost in map.getNextPoints(thePoint):
            ...
            if point not in closeList:
                ...
                if alt < g[pI]:
                    path[pI] = tpI
                    g[pI] = alt
                    f[pI] = g[pI] + map.distance(point, target)
